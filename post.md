...
## The main idea: A poor man's Google Drive for debug data
...
Filestash adds a clean web UI to your WebDAV server—think: a minimalist, local Google Drive. \
It uses Collabora Online (think: web LibreOffice) to preview office files.
...
I modified Filestash's `docker-compose.yml`[^docker-disclaimer] configuration to also set up `dufs` as our WebDAV server.
...
[^docker-disclaimer]: The docker-compose thing does not follow best practices and can be greatly improved.
I am not a DevOps engineer and even I can realize that.


## Conclusion

So, what have we learned? WebDAV + Filestash makes for a surprisingly elegant debugging solution for data-heavy applications.
